...
def everything_for_your_cat(cats_data):
    """
    Выводим строку с данными о владельце и его котах
    :param cats_data: Книга (список) о котах и покупателях
    :return: Строка с данными о владельце и его котах
    """
    # Здесь нужно написать код
    new_dict = {}  # Создаем пустой словарь
    new_list = []  # Создаем пустой список
    our_str = ''  # Создаем пустую строку, которая будет возвращать итог
    for i in cats_data:  # Пробегаем по элементам кортежа
        i = list(i)  # Преобразуем кортеж к списку, чтобы была возможность внести изменения
        i[2] += ' ' + i[3]  # К имени прибавляем фамилию, добавляя между ними "пробел"
        i.pop()  # Удаляем последний элемент списка, который соответствует фамилии владельца, которая изначально поступала на вход
        new_list.append(i)  # Добавляем в пустой список список с данными "Кличка кошки, возраст кошки, Имя и Фамилия владельца"
    for j in new_list:  # Пробегаем по элементам нового списка
        new_dict[j[2]] = new_dict.get(j[2], []) + [str(j[0]) + ',' + ' ' + str(j[1]) + ';']  # В словарь добавляем данные, где Ключ - Имя и Фамилия владельца, значение - список из Клички и Возраста кошки, разделенные ",", "пробелом", в конце ставим ";"
    new_list.clear()  # Очищаем список для его дальнейшего использования
    for key, item in new_dict.items():  # Пробегаем по ключам и значениям словаря
        our_str_1 = ''  # Создаем промежуточную строку
        our_str_1 += key + ':' + ' '  # В промежуточную строку добавляем данные из словаря, ключ + два символа ":" и пробел
        for n in item:  # Пробегаем по значениям в словаре
            our_str_1 += n + ' '  # Добавляем к промежуточной строке значения
        new_list.append(our_str_1[:-2])  # Добавляем в список строку, убрав в конце знак ";"
    for i in new_list:  # Пробегаем по элементам списка
        our_str += i + '\n'  # Добавляем элементы списка в итоговую строку, разделяя их друг от друга переносом на новую строку
    return our_str  # Возвращаем итоговую строку
