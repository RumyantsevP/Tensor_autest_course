...
def everything_for_your_cat(cats_data):
    ...
    key = ''  # Создаем пустую строку, в которой будем формировать ключи для словаря
    value = ''  # Создаем пустую строку, в которой будем формировать значения для словаря
    for i in cats_data:  # Пробегаем по элементам кортежа
        key = str(i[2]) + ' ' + str(i[3])  # Формируем ключ для дальнейшего добавления в словарь
        value = str(i[0]) + ',' + ' ' + str(i[1])  # Формируем значение для дальнейшего добавления в словарь
        if key not in new_dict.keys():  # Если ключа нет в словаре
            new_dict[key] = value  # Добавляем словарь ключ и значение
        else:  # Если ключ есть в словаре
            new_dict[key] += ';'  # Добавляем к значению разделитель ";"
            new_dict[key] += ' '  # Добавляем к значению разделитель пробел
            new_dict[key] += value  # Добавляем к текущему значению словаря новое значение
    for key, item in new_dict.items():  # # Пробегаем по ключам и значениям словаря
        our_str += key + ':' + ' ' + item + '\n'  # # Добавляем в итоговую строку ключи и значения из словаря с соответствующими разделителями
